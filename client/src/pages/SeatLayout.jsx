...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) => {
    ...
    if (selectedSeats.includes(seatId) && selectedSeats.length >= 4){
      return toast('You can select maximum 4 seats')
    }
    ...
    <div key={row} className="flex gap-2 mt-2">
      <div className="flex flex-wrap items-center justify-center gap-2">
        {Array.from({ length: count }, (_, i) => {
          ...
          return (
            <button key={seatId} onClick={() => handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-gray-400/60 cursor-pointer
             ${selectedSeats.includes(seatId) && "bg-gray-600 text-white"}
  ...
  const bookTickets = async () => {
    try {
      if(!user) return toast.error("Please login to book tickets")
        ...
        if(!selectedTime || !selectedSeats.length) return toast.error("Please select seats to book");
